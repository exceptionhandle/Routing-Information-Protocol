 ...
 */
 ...
 { \
     ...
     { \
        size += sz; \
        len -= sz; \
        cout<<"SUCCESS :: Sending PACKET ::"<<(newpack)<<endl; \
        if(len <= 0) \
        { \
           break; \
        } \
...
*/
  void resetTimers(int TIME_INTERRUPT)
  {
...
//    cout<<"RESET_TIMER ::HEAP HAS BEEN RESET"<<endl;
  ...
  int TimerInterrupt(int TIME_INTERRUPT)
  {
  ...
  {
      cout<<endl<<"START ::IF_TIME_TO_SEND_TABLE :: CHECK TIME TO SEND ROUTING"<<endl;
     if( remainTime(OwnIDinfo) <= 0 )
     {
        sendRoutingTableToNeigh();
        OwnIDinfo->Time = OwnIDinfo->period;
  ...
  void checkTimerMissed(struct IDInfo* ID)
  {
  ...
  void findShortest(unsigned char *payload, long senderID)
  {
    ...
    for(int i = 0; i < numOfNeigh; i++)
    {
        ...
        if(newCost < oldCost || (NodeInfo->nextHopID == senderID) && newCost > oldCost)
        {
            ...
            DestbyIPInfo[NodeInfo->DestIP]->nextHopID = senderID;
            DestbyIPInfo[NodeInfo->DestIP]->cost = newCost;
...
//        cout<<"END ::CONNECT_RECEIVE :: Connect and Receive the Packet "<<endl;
    ...
    {
      unsigned char *chunk = (unsigned char *)malloc(sizeof(unsigned char)*1036);
  //    cout<<"CONNECT_RECEIVE ::START :: Receiving the packet"<<endl<<endl;
      int size = 0, sz = 0;
      int len = 1036;
...
*/
              ...
              n.TimerInterrupt(TIME_INTERRUPT);
  ...
  //          cout << "SERVER ::TCP DATA Port :: Check If Packet Incoming MY IP ::" <<OwnIPAdd<< endl<<endl;
    ...
    //               cout<<" I am here REMOTE FD ::"<< remotefd<<endl;
            ...
            //     cout << "TCP_DATA :: Packet received /--/" << endl;
                 unsigned long finpad = 0;
                 unsigned long TransferID = 0, TTL = 0, SeqNum = 0;
                 char pload[1036];
                 unpack(chunk, (char*)"LCCHL", &(CHeader.controlIPAdd), &TransferID, &TTL, &SeqNum, &finpad);//, pload);
                 TTL--;
...
//          memcpy(newpack+12,(char*)subset.c_str(),1024);
          ...
          // ***************** 1 argument subset is the first index to the end of the string ******************* //
          memset(newpack+12, 0, 1024);
          memcpy(newpack+12, str+i, maxsz);
          sendTCP(sockfd, newpack);
          seq++;
        ...
        close(sockfd);
        cout<<"SEND_FILE ::"<<"NUMBER OF PACKETS ::"<<(seq-countOfPack)<<endl;
        inFile.close();
    }

    inline void sendTCP(int sockfd, unsigned char* newpack)
    {
